...
class Go2Sdk2BridgeLaunchTest(unittest.TestCase):
    ...
    def test_official_five_millisecond_fixed_speed_loop(self):
        ...
        self.assertIn("translation_speed: 0.30", self.config)
        self.assertIn("rotation_speed: 0.45", self.config)
        self.assertIn('declare_parameter("translation_speed", 0.30)', self.source)
        self.assertIn('declare_parameter("rotation_speed", 0.45)', self.source)
