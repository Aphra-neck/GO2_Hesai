...
class Go2Sdk2BridgeLaunchTest(unittest.TestCase):
    ...
    def test_official_five_millisecond_fixed_speed_loop(self):
        ...
        self.assertIn("translation_speed: 0.50", self.config)
        self.assertIn("rotation_speed: 0.75", self.config)
        self.assertIn('declare_parameter("translation_speed", 0.50)', self.source)
        self.assertIn('declare_parameter("rotation_speed", 0.75)', self.source)
        ...
        self.assertIn("persistent_arc_switch_angle: 0.04", self.config)
        self.assertIn("makeTruncatedPathSurrogate(", self.source)
        self.assertIn("selectPersistentArcSign(", self.source)
        self.assertIn("persistent_arc_sign_", self.source)
        self.assertNotIn("translation_speed_, 0.0, 0.0", self.source)
        self.assertNotIn(
