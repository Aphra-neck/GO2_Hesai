...
class Go2Sdk2BridgeLaunchTest(unittest.TestCase):
    ...
    def test_official_five_millisecond_fixed_speed_loop(self):
        ...
        self.assertIn("translation_speed: 0.40", self.config)
        self.assertIn("rotation_speed: 0.60", self.config)
        self.assertIn('declare_parameter("translation_speed", 0.40)', self.source)
        self.assertIn('declare_parameter("rotation_speed", 0.60)', self.source)
