...
当前标准 bridge 已改为直接同步刷新 `SportClient::Move`。探针中的路径进度、横向误差、
`0.05 m` 几何停车门、正向/负 `vx` 冲突等待和运动响应 watchdog 不再参与运行时
解除授权；同目标路径刷新只在控制循环中重新锚定路径游标。探针仍可用于观察
`/sdk2_command`、odom、Sport 和遥控器数据流，但 `stall` 结果不能再归因于这些已移除的
桥接器停车门。运行时仍保留 SDK 调用失败、路径/里程计输入超时、`/lowcmd` 冲突和显式停止。
